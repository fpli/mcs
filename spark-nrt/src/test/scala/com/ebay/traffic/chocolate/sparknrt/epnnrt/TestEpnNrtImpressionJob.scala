...
class TestEpnNrtImpressionJob extends BaseFunSuite{
  ...
  test("Test EPN Nrt Impression job") {
    ...
    val dedupeMeta = metadata1.readDedupeOutputMeta(".epnnrtimp")
    ...
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7457493984045429249L).select("PBLSHR_ID").first().getString(0) == "7000000007")
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7457493984045429249L).select("AMS_PBLSHR_CMPGN_ID").first().getString(0) == "-1")
  ...
  def createTestDataForEPN(): Unit = {
    ...
    metadata.writeDedupeOutputMeta(meta, Array(".epnnrtimp"))
