...
class TestBullseyeUtils_v2 extends FlatSpec with Matchers{
  ...
  "can parse response correctly" must "work" in {
    ...
    val results = BullseyeUtils_v2.getLastViewItemByResponse("1555292153683", mockResponse)
  ...
  "can parse response correctly to filter null result" must "work" in {
    ...
    val mockResponse = HttpResponse(response, 200, Map())
    val results = BullseyeUtils_v2.getLastViewItemByResponse("1555292249113", mockResponse)
  ...
  "can parse multiple response correctly" must "work" in {
    ...
    val mockResponse = HttpResponse(response, 200, Map())
    val results = BullseyeUtils_v2.getLastViewItemByResponse("1555290059224", mockResponse)
  ...
  "BullseyeUtilBasedCguid" must "work" in {
    val result = BullseyeUtils_v2.getData("6018ed8b1720a9cc5e628468f7d256a5", "910" , "200", properties.getProperty("epnnrt.bullseyeUrl"))
...
//  }
  ...
  "FetchClientSecret" must "work" in  {
    assert(BullseyeUtils_v2.getSecretByClientId(properties.getProperty("epnnrt.clientId")) == "e884b0bd-8f38-4d1d-a161-f85038c3d0f3")
  }
}
