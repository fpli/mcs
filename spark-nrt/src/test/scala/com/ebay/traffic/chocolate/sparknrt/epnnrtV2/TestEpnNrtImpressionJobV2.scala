...
class TestEpnNrtImpressionJobV2 extends BaseFunSuite{
  ...
  test("Test EPN Nrt Impression job") {
    ...
    val job = new EpnNrtImpressionJobV2(params)
    ...
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("PLCMNT_DATA_TXT").first().getString(0) == "711-53200-19255-0")
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("PBLSHR_ID").first().getDecimal(0).longValue() == 7000000007L)
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("AMS_PBLSHR_CMPGN_ID").first().getDecimal(0).longValue() == -1L)
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("AMS_TOOL_ID").first().getDecimal(0).longValue() == 10044L)
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("CSTM_ID").first().getString(0) == "1")
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("USER_QUERY_TXT").first().getString(0) == "292832042631")
    //    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("IMPRSN_TS").first().getString(0) == "2017-03-10 06:20:20.000")
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429249L).select("ROVER_URL_TXT").first().getString(0) == "http://rover.ebay.com/rover/1/711-53200-19255-0/1?ff3=2&toolid=10044&campid=5336203178&customid=1&lgeo=1&vectorid=229466&item=292832042631&raptor=1")

    // validate special case
    // filter impression from special publisher
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429241L).count() == 0)

    // rover guid fixed case
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429250L).select("GUID_TXT").first().getString(0) == "56cbd9iqoiwjddwswdwdwa33fff1c1065ad49dd7^")
    assert(impressionDf.filter(col("IMPRSN_CNTNR_ID") === 7453984045429251L).select("GUID_TXT").first().getString(0) == "56cbd9iqoiwjddwswdwdwa33fff1c1065ad49dd7^")
  }
  ...
  def createTestDataForEPN(): Unit = {
    ...
    writeFilterMessage(ChannelType.EPN, ChannelAction.IMPRESSION, 7453984045429241L, 5574651234L, -1L, "34cbd9iqoiwjddws09ydwa33fff1c1065ad49dd7^", 1489189020000L, writer1)
    ...
    writeFilterMessage(ChannelType.EPN, ChannelAction.VIEWABLE, 2902117128329247L, 7000001538L, -1L, "56cbd9iqoiwjddwswdwdwa33fff1c1065ad49dd7^", 1489159020000L,  writer2)
    writer2.close()
