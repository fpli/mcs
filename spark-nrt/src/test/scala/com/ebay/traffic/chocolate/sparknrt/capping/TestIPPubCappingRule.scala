...
import com.ebay.traffic.chocolate.sparknrt.capping.rules.IPPubCappingRule
...
class TestIPPubCappingRule extends BaseFunSuite {
  ...
  test("test ip-pub capping rule") {
    ...
    val job_01 = new IPPubCappingRule(params, CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_S), dateFiles_0, sparkJob, windowShort)
    val job_02 = new IPPubCappingRule(params, CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_L), dateFiles_0, sparkJob, windowLong)
    // handle 1st meta containing 1 meta 1 date 1 file
    val df_01 = job_01.test()
    val df_02 = job_02.test()
    df_01.show()
    df_02.show()
    assert(df_01.filter($"capping".bitwiseAND(CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_S)).=!=(0)).count() == 0)
    assert(df_02.filter($"capping".bitwiseAND(CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_L)).=!=(0)).count() == 0)
    job_01.postTest()
    job_02.postTest()
    ...
    writeFilterMessage(ChannelType.EPN, ChannelAction.CLICK, 12L, 11L, 111L, timestamp3, "1.1.1.3", writer3)
    writeFilterMessage(ChannelType.EPN, ChannelAction.CLICK, 13L, 11L, 111L, timestamp3, "1.1.1.3", writer3)
    writeFilterMessage(ChannelType.EPN, ChannelAction.IMPRESSION, 14L, 11L, 111L, timestamp3, "1.1.1.3", writer3)
    writeFilterMessage(ChannelType.EPN, ChannelAction.CLICK, 15L, 22L, 111L, timestamp3, "1.1.1.3", writer3)
    writeFilterMessage(ChannelType.EPN, ChannelAction.CLICK, 16L, -1L, 111L, timestamp3, "1.1.1.3", writer3)

    writeFilterMessage(ChannelType.EPN, ChannelAction.IMPRESSION, 17L, 11L, 111L, timestamp4, "1.1.1.2", writer4)
    ...
    assert(df_32.filter($"capping".bitwiseAND(CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_L)).=!=(0)).count() == 2)
    job_31.postTest()
    job_32.postTest()
