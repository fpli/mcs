...
class TestIPPubCappingRule extends BaseFunSuite {
  ...
  test("test ip-pub capping rule") {
    ...
    val job_0 = new IPPubCappingRule(params, CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_L), dateFiles_0, sparkJob, windowLong)
    // handle 1st meta containing 1 meta 1 date 1 file
    val df_0 = job_0.test()
    df_0.show()
    assert(df_0.filter($"capping".bitwiseAND(CappingRuleEnum.getBitValue(CappingRuleEnum.IPPubCappingRule_L)).=!=(0)).count() == 0)
    job_0.postTest()
