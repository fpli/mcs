...
class Tools(namespace: String, endpoint: String, user: String, appName: String) extends Serializable{
  @transient lazy val logger: Logger = LoggerFactory.getLogger(this.getClass)

  @transient lazy val metrics: SherlockioMetrics = {
    if(StringUtils.isNotEmpty(namespace) && StringUtils.isNotEmpty(endpoint) && StringUtils.isNotEmpty(user) && StringUtils.isNotEmpty(appName)) {
      SherlockioMetrics.init(namespace, endpoint, user)
  ...
  lazy val METRIC_IMK_DUMP_MALFORMED = "imk_dump_malformed"
  ...
  def getPerfTrackNameValue(query: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_error_getPerfTrackNameValue", 1)
        }
        logger.warn("MalformedUrl", e)
      }
    }
  ...
  def getParamValueFromQuery(query: String, key: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_error_getParamValueFromQuery", 1)
        }
        logger.warn("MalformedUrl", e)
      }
    }
  ...
  def getDefaultNullNumParamValueFromQuery(query: String, key: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_parsemtid_error", 1)
        }
  ...
  def getParamFromQuery(query: String, keys: Array[String]): String = {
    try {
      ...
      case e: Exception => {
        if (metrics != null) {
          metrics.meter("imk_dump_error_getParamFromQuery", 1)
        }
        logger.warn("MalformedUrl", e)
      }
    }
  ...
  def getUserQuery(referrer: String, query: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_errorGetQuery", 1)
        }
  ...
  def getQueryMapFromUrl(uri: String): mutable.Map[String, String] = {
    ...
    try {
      ...
      case e: Exception => {
        if (metrics != null) {
          metrics.meter("imk_dump_errorGetQueryMap", 1)
        }
  ...
  def getDecodeParamUrlValueFromQuery(query: String, key: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_error_getDecodeParamUrlValueFromQuery", 1)
        }
        logger.warn("MalformedUrl", e)
      }
    }
  ...
  def getDecodePerfTrackNameValue(query: String): String = {
    ...
    try {
      ...
      case e: Exception => {
        if(metrics != null) {
          metrics.meter("imk_dump_error_getDecodePerfTrackNameValue", 1)
        }
        logger.warn("MalformedUrl", e)
      }
    }
