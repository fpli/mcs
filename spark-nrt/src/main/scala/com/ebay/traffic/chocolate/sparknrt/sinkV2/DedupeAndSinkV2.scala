package com.ebay.traffic.chocolate.sparknrt.sinkV2
 ...
 */
object DedupeAndSinkV2 extends App {
  override def main(args: Array[String]) = {
    val params = ParameterV2(args)

    val job = new DedupeAndSinkV2(params)
...
class DedupeAndSinkV2(params: ParameterV2)
  ...
  override def run() = {
    ...
    val kafkaRDD = new KafkaRDDV2[lang.Long, FilterMessage](
   ...
   * @param writer  file writer
   */
  private def dedupeByCBAndWriteMessageV2(message: FilterMessage, writer: ParquetWriter[GenericRecord]) = {
    // couchbase dedupe only apply on click
    if ((message.getChannelAction == ChannelAction.CLICK || message.getChannelAction == ChannelAction.ROI) && couchbaseDedupe) {
      try {
