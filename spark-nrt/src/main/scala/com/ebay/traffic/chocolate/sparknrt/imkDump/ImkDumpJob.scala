...
class ImkDumpJob(params: Parameter) extends BaseSparkNrtJob(params.appName, params.mode){
  ...
  def imkDumpCore(df: DataFrame): DataFrame = {
    df
      ...
      .withColumn("user_id", getNumValueFromHeaderUdf(col("request_headers"), lit("userid")))
      ...
      .withColumn("geo_id", getNumValueFromHeaderUdf(col("request_headers"), lit("geoid")))
  ...
  val getNumValueFromHeaderUdf: UserDefinedFunction = udf((header: String, key: String) => Tools.getNumValueFromRequestHeader(header, key))
  ...
  val getUserMapIndUdf: UserDefinedFunction = udf((header: String) => Tools.getUserMapInd(Tools.getNumValueFromRequestHeader(header, "userid")))
