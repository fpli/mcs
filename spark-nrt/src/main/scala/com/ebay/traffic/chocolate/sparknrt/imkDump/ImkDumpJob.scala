...
class ImkDumpJob(params: Parameter) extends BaseSparkNrtJob(params.appName, params.mode){
  ...
  val getBrowserTypeUdf: UserDefinedFunction = udf((requestHeader: String) => Tools.getBrowserType(requestHeader).toString)
//  val getCguidUdf: UserDefinedFunction = udf((requestHeader: String, cguid: String) => Tools.getCGuidFromCookie(requestHeader, cguid))
  val getCguidUdf: UserDefinedFunction = udf((requestHeader: String, cguid: String) => Tools.getGuidFromHeader(requestHeader, cguid))
