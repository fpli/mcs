...
class TmpCompare(params: Parameter) extends BaseSparkNrtJob(params.appName, params.mode) {
  ...
  override def run(): Unit = {
    ...
    var df = ourDf.join(yourDf, $"my_uri" === $"your_uri" && $"CRLTN_GUID_TXT" === $"y_CRLTN_GUID_TXT", "inner")
      ...
      .withColumn("c_AMS_PAGE_TYPE_MAP_ID", verifyResultUdf($"AMS_PAGE_TYPE_MAP_ID", $"y_AMS_PAGE_TYPE_MAP_ID"))
      ...
      .withColumn("c_RT_RULE_FLAG3", verifyResultUdf2($"RT_RULE_FLAG3", $"y_RT_RULE_FLAG3", $"y_RT_RULE_FLAG4")).drop("RT_RULE_FLAG3")
      .withColumn("c_RT_RULE_FLAG4", verifyResultUdf2($"RT_RULE_FLAG4", $"y_RT_RULE_FLAG3", $"y_RT_RULE_FLAG4")).drop("RT_RULE_FLAG4").drop("y_RT_RULE_FLAG4").drop("y_RT_RULE_FLAG3")
    ...
    val path = new Path(params.workDir)
    fs.delete(path, true)
    saveDFToFiles(df, path.toString)
    df = readFilesAsDF(path.toString)
    val total = df.count()

    var page_df = df.where($"c_AMS_PAGE_TYPE_MAP_ID" === false && $"c_LND_PAGE_URL_NAME" === true)
    page_df = page_df.select("ROVER_URL", "LND_PAGE_URL_NAME", "AMS_PAGE_TYPE_MAP_ID","y_AMS_PAGE_TYPE_MAP_ID")

    fs.delete(new Path(path.toString + "/tmp"), true)
    saveDFToFiles(page_df, path.toString + "/tmp", "gzip", "csv", "tab")
    ...
    val AMS_PAGE_TYPE_MAP_ID = page_df.count()
