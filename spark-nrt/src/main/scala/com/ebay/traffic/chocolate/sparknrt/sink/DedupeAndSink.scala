...
class DedupeAndSink(params: Parameter)
  ...
  override def run() = {
    ...
    kafkaRDD.mapPartitions(iter => {
      ...
      while (iter.hasNext) {
        ...
        if (metrics != null) {
          metrics.meter("DedupeInputCount", 1, message.getTimestamp, Field.of(CHANNEL_ACTION, message.getChannelAction.toString), Field.of(CHANNEL_TYPE, message.getChannelType.toString))
        }
  ...
  def writeMessage(writer: ParquetWriter[GenericRecord], message: FilterMessage)  = {
    ...
    if (metrics != null) {
      metrics.meter("Dedupe-Temp-Output", 1, message.getTimestamp, Field.of(CHANNEL_ACTION, message.getChannelAction.toString), Field.of(CHANNEL_TYPE, message.getChannelType.toString))
    }
  }
}
