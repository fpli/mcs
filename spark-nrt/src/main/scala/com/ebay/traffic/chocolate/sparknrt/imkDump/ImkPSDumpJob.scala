...
class ImkPSDumpJob(params: Parameter) extends BaseSparkNrtJob(params.appName, params.mode){
  ...
  def nrtCore(df: DataFrame): DataFrame = {
    df.withColumn("EVENT_TS", getDateTimeUdf(col("timestamp")))
      ...
      .withColumn("EBAY_SITE_ID", getSiteIdUdf(col("uri")))
      ...
      .withColumn("RVR_CMND_TYPE_CD", lit(""))
      .withColumn("SRC_ROTATION_ID", getParamFromQueryUdf(col("uri"), lit("rid")))
      .withColumn("DST_ROTATION_ID", getParamFromQueryUdf(col("uri"), lit("rid")))
      .withColumn("BRWSR_NAME", getValueFromHeaderUdf(col("request_headers"), lit("User-Agent")))
      .withColumn("RVR_ID", lit(""))
...
//      .withColumn("EXTRNL_COOKIE", lit(""))
  ...
  val getSiteIdUdf: UserDefinedFunction = udf((uri: String) => Tools.getSiteIdFromRotationId(Tools.getParamValueFromUrl(uri, "rid")))
  ...
  val getUserMapIndUdf: UserDefinedFunction = udf((header: String) => Tools.getUserMapInd(header))
