...
/**
  ...
  */
class IPPubCappingRule(params: ParameterV2, bit: Long, dateFiles: DateFiles, cappingRuleJobObj: BaseSparkJob, window: String)
  ...
  override def test(): DataFrame = {
    ...
    if (head.length == 0) {
      ...
      dfIPPub = dfCappingInJob(null, ipCountPath)
