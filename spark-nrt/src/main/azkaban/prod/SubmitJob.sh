...
function submit_job_in_SLC(){
    ...
    if [ ${rcode} -eq 0 ]
    then
        start_job "slcchocolatepits-1154246.stratus.slc.ebay.com" $1
    else
        ...
        start_job "slcchocolatepits-1242730.stratus.slc.ebay.com" $1
    fi
}
...
function submit_job_in_LVS(){
    ...
    if [ ${rcode} -eq 0 ]
    then
        start_job "lvschocolatepits-1585074.stratus.lvs.ebay.com" $1
    else
        ...
        start_job "lvschocolatepits-1583703.stratus.lvs.ebay.com" $1
    fi
}
...
then
    ...
    echo "USAGE: SubmitJob.sh [CLUSTER_RUN_SPARK] [COMMAND]"
    echo 1
fi