...
/apache/hadoop/bin/hdfs dfs -mkdir -p ${APOLLO_WORK_PATH}

#save to tmp file, make the tmp file unique
tmp_file_name=rover_imk_apollo_tmp.txt
hdfs dfs -ls -R ${CHOCOLATE_HDFS_DATA_PATH} | grep -v "^$" | awk '{print $NF}' | grep "chocolate_" > ${tmp_file_name}
...
if [ ${files_size} -gt 1 ]; then
    ...
    hadoop jar chocolate-distcp-1.0-SNAPSHOT.jar -files core-site-target.xml,hdfs-site-target.xml,b_marketing_tracking_APD.keytab -copyFromInsecureToSecure -targetprinc b_marketing_tracking@APD.EBAY.COM -targetkeytab b_marketing_tracking_APD.keytab -skipcrccheck -update ${all_files} ${APOLLO_WORK_PATH}
    if [ $? -ne 0 ]; then
    ...
    do
        ...
        archive_folder=${CHOCOLATE_HDFS_DATA_ACHIVE_PATH}/dt=${date}
