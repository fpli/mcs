...
public class TrackingServlet extends HttpServlet {
  ...
  private void doRequest(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      esMetrics.meter("TrackingCount", 1, System.currentTimeMillis(), event.getAction().getAvro().toString(), event.getChannel().toString());
  ...
  private synchronized void process(HttpServletRequest request, HttpServletResponse response, TrackingEvent event) {
    ...
    try {
      ...
      esMetrics.meter("TrackingSuccess", 1, System.currentTimeMillis(), event.getAction().getAvro().toString(), event.getChannel().toString());
