...
import com.ebay.app.raptor.chocolate.filter.FilterServiceTest;
...
public class BlacklistRulesTest {
  ...
  public void testTwoPassEmpty() {
    BaseFilterWeightedRule wrule = TwoPassIABRule.createForTest(ChannelType.EPN);
    ...
    rule = wrule;
    assertEquals(1.2, rule.test(req), 0.001);
  }
  ...
  public void testWhitelist() {
    ...
    assertEquals(1.2, rule.test(req1), 0.001);
    assertEquals(1.2, rule.test(req2), 0.001);
    rule.addWhitelistEntry("bar|1|0");
    assertEquals(1.2, rule.test(req1), 0.001);
    assertEquals(0, rule.test(req2), 0.001);
    rule.clear();
  ...
  public void testIPBlacklistRule() {
    ...
    assertEquals(0, rule.test(req), 0.001);
    rule.add("foo");
    assertEquals(0.5, rule.test(req), 0.001);
    rule.clear();
    assertEquals(0, rule.test(req), 0.001);
  }
  ...
  public void testEbayDomainBlacklistRule() {
    ...
    assertEquals(0, rule.test(req), 0.001);
    rule.add("FOO");
    assertEquals(2.0, rule.test(req), 0.001);
    rule.clear();
    assertEquals(0, rule.test(req), 0.001);
