...
insert overwrite table choco_data.dw_mpx_rotations_w_mc_volatile
...
from choco_data.dw_mpx_rotations_ups mr
left outer join choco_data.dw_imk_clients ic
on ic.client_id = mr.client_id
) origin
where origin.row_number = 1;


insert overwrite table choco_data.dw_mpx_clients_temp
select mc.client_id,
       mc.client_name,
       mc.client_cntry_id,
       mc.client_site_id,
       ...
       mc.upd_date,
       mc.upd_user
from choco_data.dw_mpx_clients mc
left outer join choco_data.dw_mpx_rotations_w_mc_volatile cl
on mc.client_id = cl.client_id
where cl.client_id is null;



insert into table choco_data.dw_mpx_clients_temp
select upd.client_id,
       ...
       current_timestamp(),
       upd.upd_user
from choco_data.dw_mpx_rotations_w_mc_volatile upd
left outer join choco_data.dw_mpx_clients ori
