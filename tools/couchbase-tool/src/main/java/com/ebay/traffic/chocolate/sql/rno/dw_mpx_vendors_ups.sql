...
insert overwrite table choco_data.dw_mpx_rotations_w_ve_volatile
...
from choco_data.dw_mpx_rotations_ups upd
inner join
(select vendor_id,
...
from choco_data.dw_mpx_rotations_ups
group by vendor_id
) stat
...
insert overwrite table choco_data.dw_mpx_vendors_temp
select ven.vendor_id,
       ...
       ven.upd_date,
       ven.upd_user
from choco_data.dw_mpx_vendors ven
left outer join choco_data.dw_mpx_rotations_w_ve_volatile ve
on ven.vendor_id = ve.vendor_id
where ve.vendor_id is null;



insert into table choco_data.dw_mpx_vendors_temp
select upd.vendor_id,
       upd.vendor_name,
       upd.vendor_url,
       upd.status_id,
       ...
       current_timestamp(),
       upd.upd_user
from choco_data.dw_mpx_rotations_w_ve_volatile upd
left outer join choco_data.dw_mpx_vendors ori
...
insert overwrite table choco_data.dw_mpx_vendors
select *
from choco_data.dw_mpx_vendors_temp;