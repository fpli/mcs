...
package com.ebay.traffic.tools.util;
...
import com.ebay.traffic.tools.DistCpConstants;
import com.ebay.traffic.tools.mapred.UniformSizeInputFormat;
...
import com.ebay.traffic.tools.CopyListing.AclsNotSupportedException;
import com.ebay.traffic.tools.CopyListing.XAttrsNotSupportedException;
import com.ebay.traffic.tools.CopyListingFileStatus;
import com.ebay.traffic.tools.DistCpOptions;
import com.ebay.traffic.tools.DistCpOptions.FileAttribute;
...
public class DistCpUtils {
  ...
  /**
   * Fetch target config from distributed cache and create Configuration instance
   *
   * @return Configuration for target
   */
  public static Configuration createTargetConf() throws IOException {
    LOG.info("Initializing target configuration");

    Configuration targetConf = new Configuration(false);
    targetConf.addResource(DistCpConstants.DEFAULT_TARGET_CORE_SITE_XML);
    targetConf.addResource(DistCpConstants.DEFAULT_TARGET_HDFS_SITE_XML);
    return targetConf;
  }

  /**
   * Do Kerberos login with keytab file loaded from distributed cache
   *
   * @param conf - conf which this job runs against
   * @param targetConf - configuration for target that enables Kerberos authentication
   */
  public static void kinitTarget(Configuration conf, Configuration targetConf) throws IOException {
    LOG.info("Initializing Kerberos");

    String keytabFileName = conf.get(DistCpConstants.CONF_LABEL_TARGET_KEYTAB);
    String princ = conf.get(DistCpConstants.CONF_LABEL_TARGET_PRINC);

    // krb5.conf must already be deployed on each node in the cluster.
    UserGroupInformation.setConfiguration(targetConf);
    UserGroupInformation.loginUserFromKeytab(princ, keytabFileName);
  }
}