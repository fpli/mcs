...
public class SimpleCopyListing extends CopyListing {
  ...
  protected void validatePaths(DistCpOptions options)
    ...
    if (options.getCopyFromInsecureToSecure()) {
      // Since we assume target doesn't exist when CopyFromInsecureToSecure,
      // it doesn't make sense to do the validation here.
      return;
    }
    ...
    final boolean targetIsReservedRaw =
            Path.getPathWithoutSchemeAndAuthority(targetPath).toString().
                    startsWith(DistCpConstants.HDFS_RESERVED_RAW_DIRECTORY_NAME);
      ...
      if (Path.getPathWithoutSchemeAndAuthority(path).toString().
        ...
        if (!targetIsReservedRaw) {
          final String msg = "The source path '" + path + "' starts with " +
                  DistCpConstants.HDFS_RESERVED_RAW_DIRECTORY_NAME + " but the target path '" +
        ...
        final String msg = "The target path '" + targetPath + "' starts with " +
                DistCpConstants.HDFS_RESERVED_RAW_DIRECTORY_NAME + " but the source path '" +
   ...
   * See {@link com.ebay.traffic.tools.util.DistCpUtils#getRelativePath} for
  ...
  public void doBuildListing(SequenceFile.Writer fileListWriter,
    ...
    try {
      for (Path path: options.getSourcePaths()) {
        ...
        final boolean preserveAcls = options.shouldPreserve(DistCpOptions.FileAttribute.ACL);
        final boolean preserveXAttrs = options.shouldPreserve(DistCpOptions.FileAttribute.XATTR);
  ...
  private Path computeSourceRootPath(FileStatus sourceStatus,
    ...
    if (solitaryFile) {
      if (options.getCopyFromInsecureToSecure()) {
        // We always assume target path doesn't exist.
