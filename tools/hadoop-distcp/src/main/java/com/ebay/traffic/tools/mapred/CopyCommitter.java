...
public class CopyCommitter extends FileOutputCommitter {
  ...
  public void commitJob(JobContext jobContext) throws IOException {
    ...
    if (copyFromInsecureToSecure) {
      ...
      CommitJob(jobContext, conf, conf);
    }

    cleanup(conf);
  }

  private void CommitJob(JobContext jobContext, Configuration conf, Configuration targetConf) throws IOException {
    cleanupTempFiles(jobContext, targetConf);
    ...
    if ((attributes != null && !attributes.isEmpty()) || preserveRawXattrs) {
      preserveFileAttributesForDirectories(conf, targetConf);
    }

    if (conf.getBoolean(DistCpConstants.CONF_LABEL_DELETE_MISSING, false)) {
      deleteMissing(conf);
    } else if (conf.getBoolean(DistCpConstants.CONF_LABEL_ATOMIC_COPY, false)) {
      commitData(conf, targetConf);
    }
  ...
  public void abortJob(JobContext jobContext,
    ...
    try {
      ...
      if (copyFromInsecureToSecure) {
        ...
        cleanupTempFiles(jobContext, conf);
      }
      cleanup(conf);
    }
  }

  private void cleanupTempFiles(JobContext context, Configuration targetConf) {
    try {
      Configuration conf = context.getConfiguration();

      Path targetWorkPath = new Path(conf.get(DistCpConstants.CONF_LABEL_TARGET_WORK_PATH));
      FileSystem targetFS = targetWorkPath.getFileSystem(targetConf);
  ...
  private void preserveFileAttributesForDirectories(Configuration conf, Configuration targetConf) throws IOException {
    ...
    try {
      ...
      while (sourceReader.next(srcRelPath, srcFileStatus)) {
        ...
        FileSystem targetFS = targetFile.getFileSystem(targetConf);
  ...
  private void commitData(Configuration conf, Configuration targetConf) throws IOException {
    ...
    FileSystem targetFS = workDir.getFileSystem(targetConf);
