...
public class OptionsParser {
  ...
  private static void dumpCommand(CommandLine command) {
    for (Option opt : command.getOptions()) {
      String val = getVal(command, opt.getOpt());
      System.out.println("option: " + opt.getOpt() + (val != null ? ("=" + val) : ""));
    }
  ...
  public static DistCpOptions parse(String args[]) throws IllegalArgumentException {
    ...
    if (command.hasOption(DistCpOptionSwitch.PRESERVE_STATUS.getSwitch())) {
      ...
      if (attributes == null || attributes.isEmpty()) {
        ...
        for (int index = 0; index < attributes.length(); index++) {
          option.preserve(DistCpOptions.FileAttribute.
    ...
    if (command.hasOption(DistCpOptionSwitch.COPY_FROM_INSECURE_TO_SECURE.getSwitch())) {

      // Check the existance of principal and keytab.
      if (!command.hasOption(DistCpOptionSwitch.TARGET_PRINCIPAL.getSwitch()) ||
              !command.hasOption(DistCpOptionSwitch.TARGET_KEYTAB.getSwitch())) {
        throw new IllegalArgumentException(
                DistCpOptionSwitch.COPY_FROM_INSECURE_TO_SECURE.getSwitch() + " requires both " +
                        DistCpOptionSwitch.TARGET_PRINCIPAL.getSwitch() + " and " +
                        DistCpOptionSwitch.TARGET_KEYTAB.getSwitch() + " are specified.");
      }

      String princ = getVal(command, DistCpOptionSwitch.TARGET_PRINCIPAL.getSwitch());
      String keytab = getVal(command, DistCpOptionSwitch.TARGET_KEYTAB.getSwitch());

      option.setCopyFromInsecureToSecure(true, princ, keytab);
    }
    else {
      if (command.hasOption(DistCpOptionSwitch.TARGET_PRINCIPAL.getSwitch()) ||
              command.hasOption(DistCpOptionSwitch.TARGET_KEYTAB.getSwitch())) {
        LOG.warn(
                DistCpOptionSwitch.COPY_FROM_INSECURE_TO_SECURE.getSwitch() + " is not specified. Ignoring " +
                        DistCpOptionSwitch.TARGET_PRINCIPAL.getSwitch() + " and " +
                        DistCpOptionSwitch.TARGET_KEYTAB.getSwitch()
        );
      }
    }
