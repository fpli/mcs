...
import com.ebay.traffic.tools.CopyListing.*;
import com.ebay.traffic.tools.mapred.CopyMapper;
import com.ebay.traffic.tools.mapred.CopyOutputFormat;
...
public class DistCp extends Configured implements Tool {
  ...
  private void setTargetPathExists() throws IOException {
    if (inputOptions.getCopyFromInsecureToSecure()) {
      // Specific to Chocolate:
      // assuming distcp will run against insecure cluster, where we are not
      // able to do Kerberos authentication before the job is submitted, otherwise
      // exception will be thrown. So we can't access target file system to
      // check target path existance right here. Let's assume target doesn't
      // exist for now.
      inputOptions.setTargetPathExists(false);
      getConf().setBoolean(DistCpConstants.CONF_LABEL_TARGET_PATH_EXISTS, false);
    }
    else {
  ...
  private Job createJob() throws IOException {
    ...
    if (inputOptions.getCopyFromInsecureToSecure()) {
      configureOutputFormatWhenCopyFromInsecureToSecure(job);
    }
    else {
  ...
  /**
   * Setup output format appropriately - specific to chocolate
   *
   * @param job - Job handle
   * @throws IOException - Exception if any
   */
  private void configureOutputFormatWhenCopyFromInsecureToSecure(Job job) throws IOException {

    Path targetPath = inputOptions.getTargetPath();

    // Assume source and target file systems are compatible with both ACL and XATTR supported.
    /*
    if (inputOptions.shouldPreserve(DistCpOptions.FileAttribute.ACL)) {
      DistCpUtils.checkFileSystemAclSupport(targetFS);
    }
    ...
    */
    if (inputOptions.shouldAtomicCommit()) {
      ...
      // Assume user already specifies a valid work path on command line.
      /*
      FileSystem workFS = workDir.getFileSystem(configuration);
      if (!DistCpUtils.compareFs(targetFS, workFS)) {
        throw new IllegalArgumentException("Work path " + workDir +
