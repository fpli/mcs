...
import com.ebay.traffic.tools.mapred.CopyMapper.FileAction;
import com.ebay.traffic.tools.util.DistCpUtils;
import com.ebay.traffic.tools.util.RetriableCommand;
import com.ebay.traffic.tools.util.ThrottledInputStream;
...
public class RetriableFileCopyCommand extends RetriableCommand {
  ...
  protected Object doExecute(Object... arguments) throws Exception {
    ...
    Configuration targetConf = (Configuration)arguments[4];
    return doCopy(source, target, context, fileAttributes, targetConf);
  }

  private long doCopy(FileStatus sourceFileStatus, Path target,
                      Mapper.Context context, EnumSet<FileAttribute> fileAttributes, Configuration targetConf)
          throws IOException {
    final boolean toAppend = action == FileAction.APPEND;
    Path targetPath = toAppend ? target : getTmpFile(target, context);
    final Configuration configuration = context.getConfiguration();
    FileSystem targetFS = target.getFileSystem(targetConf);
