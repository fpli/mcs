...
class IMKClickReport(inputdir: String,
  ...
  def hourlyClickCount(header: String) = {
    ...
    val imkClickToday = onceClickCount(inputdir + "dt=" + current_date + "/*.parquet")
    val imkClickYesterday = onceClickCount(inputdir + "dt=" + getDateBeforeNDay(current_date, -1) + "/*.parquet")
    val imkClickBeforeYesterday = onceClickCount(inputdir + "dt=" + getDateBeforeNDay(current_date, -2) + "/*.parquet")
