...
/datashare/mkttracking/tools/apollo_rno/hadoop_apollo_rno/bin/hdfs dfs -ls viewfs://apollo-rno/apps/b_marketing_tracking/chocolate/epnnrt/imp/imprsn_dt=$DATE1 | grep -v "^$" | awk '{print $NF}' | grep "apollo" > ${TMP_DIR}/apollo_files/ams_impression.txt

hadoop fs -ls hdfs://elvisha/apps/epn-nrt/click/date=$DATE1 | grep -v "^$" | awk '{print $NF}' | grep "elvisha" > ${TMP_DIR}/chocolate_files/ams_click.txt
hadoop fs -ls hdfs://elvisha/apps/epn-nrt/impression/date=$DATE1 | grep -v "^$" | awk '{print $NF}' | grep "elvisha" > ${TMP_DIR}/chocolate_files/ams_impression.txt
...
/datashare/mkttracking/tools/hercules_lvs/hadoop-hercules/bin/hdfs dfs -get hdfs://hercules/apps/b_marketing_tracking/alert/epn/$DATE2/dailyClickTrend/part-*.csv ${TMP_DIR}/daily_click_trend
...
/datashare/mkttracking/tools/hercules_lvs/hadoop-hercules/bin/hdfs dfs -get hdfs://hercules/apps/b_marketing_tracking/alert/epn/$DATE2/dailyDomainTrend/part-*.csv ${TMP_DIR}/daily_domain_trend
...
java -cp /datashare/mkttracking/tools/AlertingAggrate-tool-imk-v2/lib/AlertingAggrate-tool-*.jar com.ebay.traffic.chocolate.AAMain $DATE mx.vip.ebay.com lli5@ebay.com,zhofan@ebay.com,yliu29@ebay.com,shuangxu@ebay.com,jialili1@ebay.com,xiangli4@ebay.com,fechen@ebay.com,zhiyuawang@ebay.com,zjian@ebay.com,yyang28@ebay.com,yli19@ebay.com,yuhxiao@ebay.com,xuanwwang@ebay.com daily
