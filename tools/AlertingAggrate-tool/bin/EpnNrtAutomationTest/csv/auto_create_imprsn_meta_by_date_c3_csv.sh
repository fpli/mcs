...
set -x
usage="Usage:auto_create_meta_by_date_c3_csv.sh"
...
function createMeta() {
    ...
    rm ./capping_output_*.meta.epnnrtimp
    ...
    do
      ...
      if [[ $file_index -lt 5 ]] && [[ $file_count -lt $file_total_count ]]; then
        ...
        echo "$meta_file_detail" > "capping_output_${timestamp}${file_count}.meta.epnnrtimp"
        echo "$meta_file_detail" > "capping_output_${timestamp}${file_count}.meta.epnnrtimp_v2"
      fi
    done

    echo "hdfs dfs -rm ${dest_channel_path_old_test}/capping_output_*.meta.epnnrtimp"
    hdfs dfs -rm "${dest_channel_path_old_test}/capping_output_*.meta.epnnrtimp"
    echo "hdfs dfs -put capping_output_*.meta.epnnrtimp ${dest_channel_path_old_test}"
    hdfs dfs -put capping_output_*.meta.epnnrtimp ${dest_channel_path_old_test}
