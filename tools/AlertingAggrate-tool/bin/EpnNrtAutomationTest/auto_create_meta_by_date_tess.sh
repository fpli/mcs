...
set -x
usage="Usage:auto_create_meta_by_date.sh"1

date=${date -d '2 days ago' +%Y-%m-%d}
echo "date:${date}"

export HADOOP_USER_NAME=chocolate
work_path="viewfs://apollo-rno/user/b_marketing_tracking/tracking-events"
...
function createMeta() {
    ...
    dest_channel_path_old_test="viewfs://apollo-rno/user/b_marketing_tracking/tracking-events-workdir-old-test/meta/EPN/output/capping"
    dest_channel_path_new_test="viewfs://apollo-rno/user/b_marketing_tracking/tracking-events-workdir-new-test/meta/EPN/output/capping"
    if [ ! -d "${local_channel_path}" ]; then
      mkdir -p "${local_channel_path}"
    fi
    cd "$local_channel_path"
    rm ./capping_output_*.meta.epnnrt
    /datashare/mkttracking/tools/apollo_rno/hadoop_apollo_rno/bin/hdfs dfs -ls "${work_path}/${channel}/capping/date=${date}" | grep -v "^$" | awk '{print $NF}' | grep "part" > $channel_file_list_file
