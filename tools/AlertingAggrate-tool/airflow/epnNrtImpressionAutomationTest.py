...
ams_imprsn_diff_report = SparkSubmitOperator(
    ...
    dag=dag,
    **{
        'name': "ams_imprsn_diff_report",
        'java_class': 'com.ebay.traffic.chocolate.job.AmsImprsnDiffReport',
...
ams_imprsn_diff_report.set_upstream(epnnrt_imprsn_data_parity)

epnnrt_imprsn_send_report = BashOperator(
    dag=dag,
    bash_command='/datashare/mkttracking/jobs/tracking/epnnrt_new_test/bin/epnnrt_imprsn_send_report.sh ',
    task_id='epnnrt_imprsn_send_report'
)
epnnrt_imprsn_send_report.set_upstream(ams_imprsn_diff_report)
